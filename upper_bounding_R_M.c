...
// ------------------------------------------------------------------------------------------
struct cyclotomic {
    // The prime-order cyclotomic basis, except we have the rational coordinate too:
    __uint128_t coefficients[order_of_cyclotomics];
};
struct cyclotomic multiplyBy(struct cyclotomic old_cyclo, int power) {
    // Multiplying the previous cyclotomic by a (1+\zeta^power).
    struct cyclotomic new_cyclo = old_cyclo;
    int i;
    for (i=0; i < order_of_cyclotomics - power; i++) { // First chunk of coefficient shifting
        new_cyclo.coefficients[power + i] += (__uint128_t) old_cyclo.coefficients[i];
        //printf("%d ", power + i);
    }
    //printf("\n");
    for (i=0; i < power; i++) {
        new_cyclo.coefficients[i] += (__uint128_t) old_cyclo.coefficients[order_of_cyclotomics - power + i];
        //printf("%d ", i);
    }
    //printf("\n");
    return new_cyclo;
...
// We are starting with the number 1 as a cyclotomic number repeatedly, so we will copy it from an existing source.
struct cyclotomic cyc_rational_one;
// Prepare the matrix of all 5! permutation weights that will be used for computation.
int matrix_of_weight_perms[120][5] = {
    {a, b, c, d, e},
    {a, b, c, e, d},
    {a, b, d, c, e},
    {a, b, d, e, c},
...
__uint128_t final_result = (__uint128_t) 1 << 120; // Already counted the 5-zero option.
HANDLE accum_mutex;
int last_not_done_task;
int thread_task_limit;
HANDLE task_mutex;
//HANDLE sync_semaphore;
SYNCHRONIZATION_BARRIER barrier;
//DWORD knocker_ids[order_of_cyclotomics];
//HANDLE barrier_knocker_threads[order_of_cyclotomics];
...
__uint128_t computeProductValue(int baseExponents[5]) {
    struct cyclotomic cyc_product = cyc_rational_one;
    int power_index, auxiliary_index;
    int power_in_bracket;
    for (power_index = 0; power_index < 120; power_index++) {
        ...
        //printf("%d\n", power_in_bracket);
        cyc_product = multiplyBy(cyc_product, power_in_bracket);
    }
    __uint128_t result;
    result = (__uint128_t) (order_of_cyclotomics - 1) * cyc_product.coefficients[0];
    for (auxiliary_index = 1; auxiliary_index < order_of_cyclotomics; auxiliary_index++) {
        result -= (__uint128_t) cyc_product.coefficients[auxiliary_index];
    }
    return result;
}
...
void processXEqualTo(int x) {
    ...
    __uint128_t partial_result_for_this_x = (__uint128_t) 0;
    ...
    for (y = x+1; y <= order_of_cyclotomics - 3; y++) {
        ...
        for (z = y+1; z <= order_of_cyclotomics - 2; z++) {
            ...
            if (is_minimal == 1) {
                ...
                partial_result_for_this_x += computeProductValue(five_different_base_exponents);
            }
        }
    }
    WaitForSingleObject(accum_mutex, INFINITE);
    final_result += 120 * partial_result_for_this_x;  // Gotta count 5 distinct 5-tuples.
    printf("Task x=%d finished:\t\t%016llx%016llx\n", x, (uint64_t)(final_result>>64),(uint64_t)final_result);
...
int main()
{
    ...
    // Create the "1" of cyclotomic numbers:
    int i;
    for (i = 0; i < order_of_cyclotomics; i++) {
        cyc_rational_one.coefficients[i] = 0;
    }
    cyc_rational_one.coefficients[0] = 1;
    ...
    printf("The variable final_result may need more than 128 bits!! Watch out!\n\n");
    ...
    final_result += 10*computeProductValue(base_exponents);
    ...
    final_result += 30*computeProductValue(base_exponents);
    for (z = 2; z <= (order_of_cyclotomics - 3) / 2; z++) {
        ...
        final_result += 60*computeProductValue(base_exponents);  // Multiplier here is 60, computed before.
    }
    ...
    int is_minimal = 1;
    int new_one, new_x, new_y, new_z, new_last; // Useful for the sorting and minimal orbit elements.
    ...
    for (z = 3; z <= order_of_cyclotomics - 2; z++) {
        ...
        for (y = 2; y < z; y++) {
            ...
            if (is_minimal == 1) {
                ...
                final_result += 120*computeProductValue(base_exponents);
            }
        }
    }
    ...
    final_result += 5*computeProductValue(base_exponents);
    ...
    for(i = 0; i < WORKER_COUNT; i++) {
        ...
        cubic_threads[i] = CreateThread(NULL, 0, task_picker, NULL, 0, &worker_ids[i]);
    }
    ...
    //printf("Remainder: %d \n", final_result % (__uint128_t) (order_of_cyclotomics * order_of_cyclotomics * order_of_cyclotomics * order_of_cyclotomics));
    //printf("Quotient: %d \n", final_result / (__uint128_t) (order_of_cyclotomics * order_of_cyclotomics * order_of_cyclotomics * order_of_cyclotomics));
    return 0;
}
